...
class IISAccessLogParser
  def self.fields
    @fields || %i[date server_ip method url query port username client_ip
                  user_agent user_referer host status substatus win32_status
                  time_taken other unspecified]
  end
  ...
  class Entry < OpenStruct
    def self.from_string(line)
      ...
      # x, date, server_ip, method, url, query, port, username, client_ip,
      # user_agent, user_referer status, substatus, win32_status, time_taken,
      # y, other = *line.match(/^([^ ]* [^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*)
      # ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*)($| )(.*)
      # /)
